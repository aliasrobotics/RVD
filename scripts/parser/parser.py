...
class RVDParser:
    ...
    def parse(self, content):
        ...
        self.headers = re.findall(self.MARKDOWN_HEADERS, content,  re.MULTILINE)
        self.h1_headers = re.findall(self.MARKDOWN_H1_HEADERS, content,  re.MULTILINE)
        self.links = re.findall(self.MARKDOWN_LINK, content,  re.MULTILINE)
        self.table_rows = re.findall(self.MARKDOWN_REPORT_TABLE_ROW, content,  re.MULTILINE)
        self.code_blocks = re.findall(self.MARKDOWN_CODE, content,  re.MULTILINE)
                        
        # parse RVD-specific content
        self.description = re.findall(self.RVD_DESCRIPTION, content,  re.MULTILINE)
        ...
        if self.table_rows:            
            for row in self.table_rows:
                if "rvss vector" in row[0].lower():
                    # if self.parse_rvss_vector(row[1].strip()): # TODO re-enable
                    return row[1].strip()
            return None
        return None
    ...
    # focus on specific getters which should take into account 
    # parsing
    
    # def get_property_byname(self, name):
    #     """
    #     Generic method for fetching a property from the general table
    ...
    # #NOTE: logic moved to Malformed class    
    # def parse_cwe_id(self, cwe_id):
    #     """
    #     Parse and validate CWE ID
    # 
    #     :param (string) cwe_id
    #     :return bool
    #     """
    ...
    # 
    # def parse_cve_id(self):
    #     """
    #     Parse and validate CVE ID
    #     """
    #     raise NotImplementedError("TODO")
    # 
    # def parse_rvss_vector(self):
    #     """
    #     Parse and validate RVSS vector against the RVSS score.
    #     """
    #     raise NotImplementedError("TODO")
    # 